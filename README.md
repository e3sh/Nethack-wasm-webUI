...
- **全画面マップ ＋ イマーシブHUD (Neo-Retro Dark Glass UI)**:  
  100vw × 100vh の全画面ダンジョンビューに、フロストガラス（透過ブラー ＋ 極細ボーダー）とサイバーシアンのアクセントカラーで統一された本格PCゲームライクなHUDを配置。視界を遮らないフローティングメッセージHUD、キー操作（`L`）やワンタップで展開できる過去ログドロワー（画面左端📌固定対応）、サイドパネルのクイック一時退避・ワンタップ復帰（`Alt+S` / `[` / 画面右端Peekタブ）を搭載。
- **ペーパードール装備管理**:  
