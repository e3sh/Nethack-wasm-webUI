...
import { ITEM_INTERACTION_RULES, evaluateInteractionRule } from "../data/ITEM_INTERACTION_RULES.js";
import { isShopkeeperMonster } from './glyphClassifier.js';
import { createAdvice } from "../data/ADVICE_DEFINITIONS.js";
import { EquipmentDependencyAnalyzer } from "../equipment/EquipmentDependencyAnalyzer.js";
import { isCockatriceCorpse } from "../equipment/EquipmentRules.js";
...
export class TacticalAdvisor {
    ...
    static evaluateEquipmentAdvices(inventoryState, skillStateManager, advices) {
        ...
        // 現在装備中の武器 (左利き・右利き両対応)
        const currentWielded = meleeWeapons.find(w => w.isWielded || (w.rawText && /(?:\bweapon in (?:hands?|right hand|left hand)\b|\bwielded\b|[左右両]?手に装備中|[左右両]?手に持っている|[\(（]装備中[\)）])/i.test(w.rawText)));
