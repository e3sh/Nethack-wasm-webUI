...
### 4.2. MML (Music Macro Language) 記法による音程・メロディ指定

`beep` オブジェクト内の `notes`（または `mml`）プロパティでは、従来の配列形式（`["C4", "E4", "G4"]`）に加え、**MML風文字列** による直感的なメロディ記述に対応しています：

#### MML 記載例
```json
"beep": {
  "mml": "T180 O4 L8 C E G O5 C.",
  "wave": "square"
}
```

* **対応MMLコマンド一覧**:
  * `T10` 〜 `T600`: テンポ指定 (BPM: Beats Per Minute)。$\text{L4の基準時間 (ms)} = \frac{60000}{\text{BPM}}$ により、各音符のミリ秒長さを動的自動計算します。曲の途中で複数回 `T` を指定して速度を変えることも可能です（例: `"T120 C D E T240 F G A"`）。※ `T` 未指定時は `duration`（デフォルト 80ms）が採用されます。
  * `O1` 〜 `O9`: オクターブ指定 (デフォルト: `O4`)
  * `L1` 〜 `L64`: デフォルト音長指定 (`L4`: 4分音符, `L8`: 8分音符 等)
  * `V0` 〜 `V15`: 音量指定 (0 〜 15.0)
  * `A` 〜 `G`: 音名指定 (`#` / `+` で半音上げ、音符直後に長さ数値指定可能 例: `C8`, 付点 `.` 対応)
  * `R`: 休符 (Rest)
  * カンマ区切り/スペース区切り文字列 (`"C4 E4 G4"` / `"C4, E4, G4"`) や配列形式も自動認識されます。
