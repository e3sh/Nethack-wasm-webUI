...
### 2.3 静的マスターの初回遅延固定化 (Lazy Memoization) と動的ステート合成
- **初回オンデマンド固定化 (Lazy Memoization)**:
  - 起動時の全件翻訳による待ち時間（起動遅延）を発生させず、ゲーム中にアイテム・モンスター・地形が初めて要求された瞬間にのみ 1 度ローカライズして正規化キー（`ja_item_${onum}`, `ja_mon_${id}`, `ja_terrain_${id}`）でキャッシュ固定。
  - 2回目以降のインベントリ同期やマップホバーでは、翻訳エンジン（`TranslationEngine.translate`）へのリクエストが完全ゼロ（0ms・0回）となり、GC負荷も極小化。
- **動的ステートの安全な上乗せ合成**:
  - 識別状態（未識別/仮名/識別済み）、BUC（呪い/通常/祝福）、強化値、装備状態、ペット/敵対フラグ、確定HPなどの可変情報は固定キャッシュせず、キャッシュされた静的マスターの上にリアルタイムにシャローコピー合成して返却。
  - これにより、情報の古延び（不整合）リスクを排除しながら最高のレンダリングパフォーマンスを実現。
