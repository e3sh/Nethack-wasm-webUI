...
## 7. 所持品アイテムのワンタッチ自走アクションシーケンス規定

`InventoryStateManager` が各アイテムに付与する `defaultSequence`（ワンタップ自走用キー配列）の標準規定です。

| 対象アイテムカテゴリ | 状態・条件 | 送出シーケンス (`defaultSequence`) | 挙動説明 |
|---|---|---|---|
| **ポーション (Potion)** | 全般 (未識別色含む) | `['q', letter]` | ポーションを飲む (`q`) ➔ アイテム指定 |
| **食品 (Food)** | 全般 (死体・缶詰等) | `['e', letter]` | 食品を食べる (`e`) ➔ アイテム指定 |
| **巻物 / 呪文書** | 全般 | `['r', letter]` | 読む (`r`) ➔ アイテム指定 |
| **杖 (Wand)** | 全般 | `['z', letter]` | 振る (`z`) ➔ アイテム指定 |
| **道具 (Tool)** | タオル, 鍵, ツルハシ等 | `['a', letter]` | 使う (`a`) ➔ アイテム指定 |
| **未着用防具** | 未着用 | `['W', letter]` | 着用する (`W`) ➔ アイテム指定 |
| **着用中防具** | 着用中 | `['T', letter]` | 脱ぐ (`T`) ➔ アイテム指定 |
| **未装着指輪** | 左手空き | `['P', letter, 'l']` | 指輪装着 (`P`) ➔ アイテム指定 ➔ 左手 (`l`) 自動補完 |
| **未装着指輪** | 左手埋まり・右手空き | `['P', letter, 'r']` | 指輪装着 (`P`) ➔ アイテム指定 ➔ 右手 (`r`) 自動補完 |
| **装着中指輪/お守り** | 装着中 | `['R', letter]` | 外す (`R`) ➔ アイテム指定 |
| **未装備武器** | 未装備 | `['w', letter]` | 手に持つ (`w`) ➔ アイテム指定 |
| **装備中武器** | 装備中 | `['w', '-']` | 手放す (`w`) ➔ 素手 (`-`) 指定 |
| **未知/判定不能品** | 変名等 (`OTHER`) | `['i', letter]` | インベントリ表示 (`i`) ➔ アイテム指定 (移動暴発安全防止) |
