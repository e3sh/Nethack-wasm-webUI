...
- **FIFO タスクキュー管理（シリアル非破棄実行）**:
  - `queueSequence` に投入された各シーケンスは、固有の表示オプションやバッファを伴うタスクオブジェクト `{ tokens, options, buffer }` として **FIFO（先入れ先出し）タスクキュー** に保持されます。
  - 先行するシーケンスの自走消化中や入力待ち中に新しい `queueSequence` が呼び出された場合でも、先行するシーケンスを途中で破棄・上書きすることはありません。先行タスクが完了し Cコアの処理が空いたタイミングで、次のタスクを自動的に引き継いで順次安全に開始します。
- **SoC (責務の分離) 原則**:
