...
### 4.4 武器・手周りの設計: 「正副武器切替（x）＆ 二刀流トグル（X）」
- **主手 (Main Hand)**:
  - 武器、死体、ツルハシなどを受け入れる（`w [letter]` を発行）。
  - 空にする場合は `w -` を発行。
  - 両手武器（Two-handed）を装備する場合、現在 `off_hand` に盾があれば「盾を脱ぐ」をブロッカーに登録。
- **正副武器切替ボタン（Swap Weapon: `x`）**:
  - 主手スロット（`MAIN_HAND`）と控えスロット（`OFF_HAND`）の間に配置。
  - クリック時に NetHack 標準の `x` コマンド（武器スワップ）を発行し、主手と控えを瞬時に交換して即座にダイアログを再描画。
- **二刀流トグルボタン（Two-Weapon Combat: `X` / `#twoweapon`）**:
  - 控えスロット（`OFF_HAND`）の直下に配置。
  - **適性判定**: 二刀流適性職業（Samurai, Barbarian, Rogue, Ranger, Knight 等）またはスキル熟練度で `two-weapon combat` が制限（Restricted）されていない場合に活性化（非適性時は `disabled`）。
  - **状態ハイライト**: 装備状態に `isOffhand`（副武器）が存在する二刀流戦闘中はボタンに `is-active` クラス（金枠・ゴールド点灯グロー）を付与。
  - クリック時に NetHack 標準の `X` コマンドを発行し、二刀流の ON/OFF を切り替えて即座にダイアログを再描画。
- **副手 (Shield / Off Hand)**:
  - 通常は「盾」スロット（`W` / `T`）。両手武器装備中および二刀流中は配置不可。
- **矢筒 (Quiver)**:
