...
* **オーバーレイレイヤー（前面・透過浮遊 HUD）**:
  * **メッセージログ (★アプローチB採用)**: 画面上部にフローティング最新行HUDを配置し、`Ctrl+P` やスクロールで展開する過去ログドロワーを新設。詳細は **[immersive_hud_message_window_specification.ja.md](./immersive_hud_message_window_specification.ja.md)** を参照。
