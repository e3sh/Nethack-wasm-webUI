...
## 8. 共通ナレッジコンポーネント (GKL) 構築規格

すべてのサンプルクライアント（Vue, React, Svelte, SolidJS）には、同一の機能と高いデザイン性を担保した **`GklKnowledgePanel`** コンポーネントを配置します。

1. **⚡ アイコン即時自動実行**: 所持品アイコンタップで `executeSequence(['w', letter])` または `[letter]` 一発実行。
2. **💡 浮き出しポップオーバー**: アイコンホバー時に「アイテム名」「ワンタップ予告」「日本語効果サマリー」を浮き出し表示。
3. **🎽 装備バッジ ＆ 枠線カラー**: メイン武器 (`[手]`), 副武器 (`[副]`), 矢筒 (`[筒]`), 着用防具 (`[着]`)。
4. **🎯 8方向アクションフィルター**: `extractDirectionCode(act)` による方向別フィルタリングおよび件数バッジ。
5. **🔍 7x7 ダンジョンズームカメラ**: プレイヤー中心の 7x7 ミニマップ（24px格子スプライト）。`tileId === 0` かつ `symbol === ' '` の未探索セルにおける `giant ant` 誤検出防止ロジックの搭載。

---

## 9. まとめ

本ドキュメントに定めたルールを遵守することで、各種コンポーネントフレームワーク（Vue 3, React 18, Svelte, SolidJS）のサンプルクライアント開発において、手戻りのない堅牢な設計と、`WebUICore` の独立性を損なうことなく、高品質で均一な NetHack UI をスピーディに構築・拡張できます。
