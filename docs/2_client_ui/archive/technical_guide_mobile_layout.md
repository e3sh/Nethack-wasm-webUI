...
## 5. プラットフォーム別のフルスクリーン制約

ブラウザとOSの組み合わせにより、Fullscreen API の挙動は大きく異なります。

| プラットフォーム | Fullscreen API | ユーザー操作要件 | 特記事項 |
| :--- | :--- | :--- | :--- |
| **PC (Chrome/Edge/FF)** | フルサポート | 必須 (Click/Key) | 標準的で安定した挙動。 |
| **Android (Chrome)** | フルサポート | 必須 (Click) | PWA化でアドレスバーが消え、最も没入感が高い。 |
| **iPadOS (Safari)** | プレフィックス必要 | 必須 (Click) | `webkitRequestFullscreen` が機能する。PWA化も有効。 |
| **iOS / iPhone** | **非サポート** | - | 動画以外は全画面化不可。PWA化が唯一の手段。 |

### 実装上の注意点
*   **ハングアップ回避:** API非対応環境（iPhone等）で `requestFullscreen()` を呼ぶと実行時エラーになります。必ず `fullscreenEnabled` のチェックと `try-catch` / `Promise.catch()` で保護する必要があります。
*   **全画面でのUI考慮:** 全画面モード（特にPWA）では、ブラウザの「更新」「戻る」ボタンが消失します。ゲームオーバー後に再開できなくなるのを防ぐため、アプリ側でリロード機能（`location.reload()`）を用意するなどの配慮が不可欠です。
