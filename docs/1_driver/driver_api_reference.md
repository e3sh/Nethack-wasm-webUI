...
- **特徴・安全ガード**:
  - **メニュー選択 (`select_menu`) のトークン自動解決**: メニュー入力待ち時にトークンが渡された場合、アクセラレータキー（`'a'`, `'B'` 等）、アイテム文字コード、数値インデックス（1-based）、またはキャンセル値（`27`, `'0'`）を自動判定して対応するメニュー項目オブジェクトを生成・Cコアへ返却します。これにより `queueSequence(['i', 'a'])` 等による 2段目メニューへの自動遷移が安全に行えます。
