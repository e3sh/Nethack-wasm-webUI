...
本フォルダ（`docs/9_translation/`）には、NetHack Wasm WebUI 移植プロジェクトにおける翻訳システム、辞書データ運用、支援ツールの使用法に関する説明ドキュメントが格納されています。
...
### 6. 🚀 [翻訳システム次世代刷新・拡張設計構想](translation_architecture_enhancement_plan.md)
> **動的かすれ・コンテキスト分類・多段パイプライン化構想**
> 
> - **内容**: 地面の文字かすれ（Engraving）の復元、マルチバイト文字化け防止、小分類（Category）の自動付与による誤訳防止と段階的刷新プラン。
> - **対象**: 翻訳エンジンの将来的な機能拡張・辞書データ構造の改善を検討する開発者。
