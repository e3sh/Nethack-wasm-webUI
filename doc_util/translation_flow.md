...
-   **実行中に未翻訳になった文章の確認方法**:　`LANG＿LARNMODE`が`true`で実行している場合に、`tr_fail.html`で翻訳実行されずにスルーされたメッセージが確認できるようになっています。
(`LANG＿LARNMODE`は、`rogueDefines.js`の11行目で設定)
`LANG_JP`を`false`にすると翻訳処理を未実施のまま原文を表示するようになります。
